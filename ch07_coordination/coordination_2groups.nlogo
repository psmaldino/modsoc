;; An asymmetric coordination model in two groups. Everyone starts with one of two norms.
...
to setup
  clear-all
  color-patches ;;divide the space into group A and group B territory
...
to go
  ...
  if (count turtles with [norm1?]) = (count turtles)  ;or (count turtles with [norm1? and groupID = 1]) = 0
    [ stop ]
...
This model is original material created by Paul E. Smaldino. The model is based on work by Boyd and Richerson (2002). 
...
* Smaldino PE (2023). Coordination: Two Groups. Modeling Social Behavior.  https://github.com/psmaldino/modsoc/
...
<experiments>
  ...
  <experiment name="experiment2" repetitions="100" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="norm1-prosocial-benefit" first="0" step="0.5" last="8"/>
